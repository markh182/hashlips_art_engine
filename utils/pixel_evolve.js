...
const buildDir = `${basePath}/build/pixel_evolve_gifs`;
const inputDir = `${basePath}/build/images`;
const { format, pixelEvolve } = require(path.join(basePath, "/src/config.js"));
...
const getImages = (_dir) => {
  try {
    return fs.readdirSync(_dir).filter((item) => {
...
const draw = (_imgObject, _ratio) => {
  let size = _ratio.ratio;
...
const startCreating = async () => {
  ...
  const { ratios, repeat, quality, delay } = pixelEvolve;
  ...
  await Promise.all(loadedImageObjects).then((loadedImageObjectArray) => {
    loadedImageObjectArray.forEach((loadedImageObject) => {
      ...
      hashlipsGiffer = new HashlipsGiffer(
        canvas,
        ctx,
        `${buildDir}/${loadedImageObject.imgObject.filename.split('.')[0]}.gif`,
        repeat,
        quality,
        delay
      );
